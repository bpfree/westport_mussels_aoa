...
mec <- sf::st_read(dsn = submodel_gpkg,
                   layer = sf::st_layers(dsn = submodel_gpkg)[[1]][grep(pattern = stringr::str_glue("explosives_{date}"),
                                                                        x = sf::st_layers(dsn = submodel_gpkg)[[1]])])

### danger zones
danger_zones <- sf::st_read(dsn = submodel_gpkg,
                   layer = sf::st_layers(dsn = submodel_gpkg)[[1]][grep(pattern = stringr::str_glue("restricted_areas_{date}"),
                                                                        x = sf::st_layers(dsn = submodel_gpkg)[[1]])])

### environmental sensors
danger_zones <- sf::st_read(dsn = submodel_gpkg,
                            layer = sf::st_layers(dsn = submodel_gpkg)[[1]][grep(pattern = stringr::str_glue("sensor_{date}"),
                                                                                 x = sf::st_layers(dsn = submodel_gpkg)[[1]])])
