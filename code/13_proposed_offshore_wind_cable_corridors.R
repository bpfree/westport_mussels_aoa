##################################################
### 13. Proposed offshore wind cable corridors ###
##################################################
...
### input directories
#### cable corridors
data_dir <- "data/a_raw_data/Offshore_Wind-_Proposed_Export_Cable_Corridors"
...
region_data <- data %>%
  # obtain only cable corridors in the study area
  rmapshaper::ms_clip(target = .,
                      clip = region) %>%
  # create field called "layer" and fill with "cable corridors" for summary
  dplyr::mutate(layer = "cable corridors")

#####################################
#####################################

# cable corridors hex grids
region_data_hex <- hex_grid[region_data, ] %>%
  # spatially join cable corridors values to Westport hex cells
  sf::st_join(x = .,
              y = region_data,
              join = st_intersects) %>%
